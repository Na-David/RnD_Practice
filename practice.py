...
class Evaluator:
                    ...
                    if self.opt.benchmark == 'MOT16':
                        for member in tqdm(zip_file.namelist(), desc=f'Extracting {benchmark}'):
                            ...
                            if not member_path.exists() and not member_path.is_file():
                                zip_file.extract(
                                    member, val_tools_path / 'data' / 'MOT16')
                    else:
                        for member in tqdm(zip_file.namelist(), desc=f'Extracting {benchmark}'):
                            member_path = val_tools_path / 'data' / member
                            if not member_path.exists() and not member_path.is_file():
                                zip_file.extract(
                                    member, val_tools_path / 'data')
                LOGGER.info(f'{benchmark}.zip unzipped successfully')
            except Exception as e:
                print(
                    f'{benchmark}.zip is corrupted. Try deleting the file and run the script again')
                sys.exit()
    ...
    def eval_setup(self, opt, val_tools_path):
        ...
        if opt.benchmark == 'MOT17':
            ...
            mot_seqs_path = Path('./assets') / \
                self.opt.benchmark / self.opt.split
        ...
        MOT_results_folder = val_tools_path / 'data' / 'trackers' / \
    ...
    def eval(self, opt, seq_paths, save_dir, MOT_results_folder, val_tools_path, gt_folder, free_devices):
        ...
        if not self.opt.eval_existing:
            ...
            for i, seq_path in enumerate(seq_paths):
                ...
                if i > 0 and len(free_devices) == 0:
                    if len(processes) == 0:
                        raise IndexError(
                            "No active processes and no devices available.")
        ...
        results = (save_dir.parent / self.opt.eval_existing /
                   'tracks' if self.opt.eval_existing else save_dir / 'tracks').glob('*.txt')
        for src in results:
            if self.opt.eval_existing:
                dst = MOT_results_folder.parent.parent / \
                    self.opt.eval_existing / 'data' / Path(src.stem + '.txt')
            else:
        ...
        p = subprocess.run(
            args=[
                ...
                "--USE_PARALLEL", "True",
                "--TRACKER_SUB_FOLDER", str(Path(*
                                                 Path(MOT_results_folder).parts[-2:])),
        ...
        shutil.copyfile(opt.tracking_config, save_dir /
                        opt.tracking_config.name)
    ...
    def parse_mot_results(self, results):
        ...
        combined_results = {key: value for key, value in zip(
            ['HOTA', 'MOTA', 'IDF1'], combined_results)}
    ...
    def run(self, opt):
        ...
        seq_paths, save_dir, MOT_results_folder, gt_folder = e.eval_setup(
            opt, val_tools_path)
        free_devices = e.device_setup(opt, seq_paths)
        results = e.eval(opt, seq_paths, save_dir, MOT_results_folder,
                         val_tools_path, gt_folder, free_devices)
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', type=str,
                        default=WEIGHTS / 'yolov8n.pt', help='model.pt path(s)')
    parser.add_argument('--reid-weights', type=str,
                        default=WEIGHTS / 'osnet_x1_0_dukemtmcreid.pt')
    parser.add_argument('--tracking-method', type=str,
                        default='strongsort', help='strongsort, ocsort')
    parser.add_argument('--tracking-config', type=Path, default=None)
    parser.add_argument('--name', default='exp',
                        help='save results to project/name')
    parser.add_argument('--project', default=ROOT / 'runs' /
                        'val', help='save results to project/name')
    parser.add_argument('--exist-ok', action='store_true',
                        help='existing project/name ok, do not increment')
    parser.add_argument('--benchmark', type=str,
                        default='MOT17', help='MOT16, MOT17, MOT20')
